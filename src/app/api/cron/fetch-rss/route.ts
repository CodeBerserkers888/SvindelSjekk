...
const RSS_SOURCES = [
  { name: "Regjeringen.no", url: "https://www.regjeringen.no/no/rss-feeder/nyheter/id2004965/", category: "advarsel" },
  { name: "Forbrukerrådet", url: "https://www.forbrukerradet.no/feed/", category: "advarsel" },
  { name: "Datatilsynet", url: "https://www.datatilsynet.no/rss/", category: "advarsel" },
  { name: "Finans Norge", url: "https://www.finansnorge.no/rss/nyheter/", category: "svindel" },
];
  ...
  for (const source of RSS_SOURCES) {
    try {
      const res = await fetch(source.url, {
        headers: { "User-Agent": "SvindelSjekk RSS Bot/1.0" },
      ...
      for (const item of items) {
        ...
        const inserted = await insertPost({
          ...
          content: `${item.description}\n\nKilde: ${source.name} — ${item.link}`,
          category: source.category,
          published_at: new Date(item.pubDate).toISOString(),
        });
        if (inserted) results.inserted++;
        else results.errors.push(`Failed: ${item.title}`);
      }
    } catch (err) {
      results.errors.push(`${source.name}: ${String(err)}`);
    }
  }
