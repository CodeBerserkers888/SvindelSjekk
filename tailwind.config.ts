...
Et initiativ av **Trygg På Nett** — beskytt deg og dine mot svindel på nett.
...
| 📰 **Nyheter & advarsler** | Automatisk oppdatert blogg med siste svindelforsøk i Norge |
| 🤖 **RSS auto-fetch** | Henter nyheter fra NRK, Forbrukerrådet og Aftenposten hver time |
...
(svindel, phishing, bedrageri...)
        │
        ▼
Supabase blog_posts
        │
        ▼
Vises automatisk på /nyheter
```
...
| **Domene** | svindelsjekk.no |
| **PWA** | Service Worker, Web App Manifest |
| **Cron** | Vercel Cron Jobs (hver time) |
...
# Cron security
CRON_SECRET=din_hemmelige_nokkel
```
...
│   │   ├── blog/               ← Blogg-API
│   │   └── cron/
│   │       └── fetch-rss/      ← Auto RSS-henting (hver time)
│   ├── nyheter/                ← Svindelnyheter (auto-oppdatert)
│   │   └── [slug]/             ← Enkeltartikkel
...
- [x] RSS auto-fetch (Vercel Cron, hver time)
- [x] PWA — installer som app
- [x] Rate limiting
- [x] Egen domene svindelsjekk.no
- [x] Trygg På Nett merkevare
- [ ] AI — intelligent tekstanalyse
- [ ] Admin-panel for blogginnlegg
- [ ] Analyse av telefonnumre
- [ ] Push-varsler for nye advarsler
...
Et initiativ av **Trygg På Nett** — laget med ❤️ for å beskytte nordmenn mot svindel
